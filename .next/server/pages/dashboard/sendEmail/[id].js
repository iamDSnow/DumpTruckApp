    ...
    function getD() {
        new Promise((resolve)=>{
            setTimeout(()=>{
                ...
                }).join("")), setDriverLicInput(data.data.Users.map((user)=>{
                    if (user.uid === id) {
                        return user.driverLic;
                    }
                }).join("")), setTruckNumberInput(data.data.Users.map((user)=>{
                    if (user.uid === id) {
                        return user.truckPlateNumber;
                    }
                }).join("")), setPhoneInput(data.data.Users.map((user)=>{
                    if (user.uid === id) {
                        return user.phone;
                    }
                }).join("")), setCompanyInput(data.data.Users.map((user)=>{
                    if (user.uid === id) {
                        return user.company;
                    }
                }).join("")), st.getTime(startTimeInput), et.getTime(endTimeInput), setLoadIdInput(data.data.Users.map((user)=>{
                    if (user.uid === id) {
